...
class TradingEnv(gym.Env):
    ...
    def __init__(self, marketSymbol, startingDate, endingDate, money, stateLength=30,
        ...
        # CASE 2: Real stock loading
            ...
            if(exists):
                self.data = CSVToDataframe(csvName)
            # Otherwise, download the stock market data from Yahoo Finance and save it in the database
            else:  
                self.data = getDailyData(marketSymbol, startingDate, endingDate)
            if saving == True:
                dataframeToCSV(csvName, self.data)
        ...
        # self.data.ffill(0, inplace=True)
    ...
    def step(self, action):
        ...
        if(action == 1):
            self.data['Position'].iloc[t] = 1
            # Case a: Long -> Long
            if(self.data['Position'].iloc[t - 1] == 1):
                self.data['Cash'].iloc[t] = self.data['Cash'].iloc[t - 1]
                self.data['Holdings'].iloc[t] = self.numberOfShares * self.data['Close'].iloc[t]
            # Case b: No position -> Long
                ...
                self.data['Cash'].iloc[t] = self.data['Cash'].iloc[t - 1] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.data['Holdings'].iloc[t] = self.numberOfShares * self.data['Close'].iloc[t]
                self.data['Action'].iloc[t] = 1
            # Case c: Short -> Long
            else:
                self.data['Cash'].iloc[t] = self.data['Cash'].iloc[t - 1] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.numberOfShares = math.floor(self.data['Cash'].iloc[t]/(self.data['Close'][t] * (1 + self.transactionCosts)))
                self.data['Cash'].iloc[t] = self.data['Cash'].iloc[t] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.data['Holdings'].iloc[t] = self.numberOfShares * self.data['Close'].iloc[t]
                self.data['Action'].iloc[t] = 1

        # CASE 2: SHORT POSITION
        elif(action == 0):
            self.data.loc[t,'Position'] = -1
            # Case a: Short -> Short
            if(self.data['Position'].iloc[t - 1] == -1):
                ...
                if lowerBound <= 0:
                    ...
                    self.data['Cash'].iloc[t] = self.data['Cash'].iloc[t - 1] - numberOfSharesToBuy * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                    self.data['Holdings'].iloc[t] =  - self.numberOfShares * self.data['Close'].iloc[t]
                    customReward = True
        ...
        self.data['Money'].iloc[t] = self.data['Holdings'].iloc[t] + self.data['Cash'].iloc[t]
        self.data['Returns'].iloc[t] = (self.data['Money'].iloc[t] - self.data['Money'].iloc[t-1])/self.data['Money'].iloc[t-1]
